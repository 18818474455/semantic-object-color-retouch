...
- **C3-4 (in progress)**: `eval_harmony.py` (done) — automated metrics (fg/bg luma diff, boundary ΔE, skin hue drift, highlight/shadow clip, region pair confidence) computed from the SAME `analyze_target()` output for both pipelines so comparisons aren't confounded by re-segmenting. Ran on all 20 existing samples: coherence vs legacy — flags -51%, fg/bg luma diff -81%, boundary ΔE p95/p99 -44%/-40%, skin hue drift -33%, highlight/shadow clip roughly flat. `--summarize --out-root <dir>` aggregates existing metrics.json without re-rendering. See `outputs/phase-c3-4-eval-harmony.md`. **NOT done**: `FG-BG-Coord-v1` is still only 20 samples (need ≥30, especially the still-unlocated "steel-truss-ceiling + dense crowd" original bug photo — ask the user for its path rather than substituting a similar-looking photo); `review.json` per sample is still an empty template — nobody has scored anything yet. Do not claim C3-4 complete or start C3-5/C3-6 until both are done and the plan doc §四 acceptance thresholds are actually evaluated against real human scores.

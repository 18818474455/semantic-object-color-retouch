...
- **C3-3 (next)**: replace the current fixed-radius feathered-mask blend with residual-level edge-aware fusion (Guided Filter guided by target luminance); enrich `_region_homogeneity_confidence` with texture/edge-density features (currently Lab-variance only).

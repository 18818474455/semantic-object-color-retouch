...
- **C3-4 (next)**: implement `eval_harmony.py` automated metrics; grow `FG-BG-Coord-v1` from 20 to ≥30 samples (prioritize dense-crowd+truss-ceiling, treeline+sky, white/light clothing, weak ref-target content match); run the manual 1-5 scoring pass; only proceed to C3-5/C3-6 (rebuild C2 data, resume M3.7) once acceptance criteria in the plan doc §四 are met.

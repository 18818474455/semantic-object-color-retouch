...
Phase C1c — 本地 VLM 语义门控/critic（新增，辅助轨）
  目的：补齐 V3 §4.2 一直缺失的"VLM 交叉验证"格子，替代/校验启发式天空合理性规则；
        顺带给 C1 提供一个不依赖第三方 API 的本地 teacher/critic 备选
  依据：ModelScope VLM 盘点核实 Qwen3-VL 全系列 Apache-2.0，商用无门槛
        （09-行业方案与知识库/App/2026-07-09-ModelScope图文多模态VLM模型盘点.md）
  验证：Qwen3-VL-8B-Instruct 在 20 图回归集上对比现有启发式规则的判断准确率
  状态：未开始，待排期实验
...
1. **C1c 实验**：Qwen3-VL-8B-Instruct 在 20 图回归集上对比现有启发式天空合理性规则，看能否替代/简化 §4.2 一直缺失的 VLM 交叉验证  
2. **升级模型**：n=208 已足够，把 `train_per_class_head.py` 从 ridge baseline 升级为轻量 torch MLP  
3. **M3.7**：在 Chroma 仓开 `feature/regional-smart-color-head` 分支（数据门槛已达标，可以启动 C2.4 嫁接）  
4. **C1** API易 双图冒烟（并行，不阻塞 1–3）  
5. **Web Demo**（Polarr 式三控件）基于 `color_reference_transfer.py`  
6. 与 Chroma 仓对齐 **M3.7 masked operator** 排期  

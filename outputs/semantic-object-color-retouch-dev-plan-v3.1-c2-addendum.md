...
1. **M3.7**：在 Chroma 仓开 `feature/regional-smart-color-head` 分支（数据门槛已达标，可以启动 C2.4 嫁接）  
2. **C1** API易 双图冒烟（并行，不阻塞 1）  
3. **Web Demo**（Polarr 式三控件）基于 `color_reference_transfer.py`  
4. 与 Chroma 仓对齐 **M3.7 masked operator** 排期  
5. 数据量进一步扩大后（更多 Stage 0/新样本，或 C1 GPT hard-case 补充）重跑 `train_per_class_head_mlp.py` 复查 MLP 是否反超  

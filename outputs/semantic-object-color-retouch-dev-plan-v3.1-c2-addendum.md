...
- [x] **排查并修复同标签类别外观差过大问题**：用户反馈"背景跟前景严重脱节"；开放词汇标签把两种物理不同的东西（商场白吊顶 vs 钢架顶棚）都标成"building"强行统计匹配，新增 `_class_pair_confidence` 按绝对 Lab 差距压制过冲；**第三次重跑 C2.1→C2.3**，新 held-out MAE=3.74（真实改善），见 `phase-teacher-class-mismatch-fix.md`  
